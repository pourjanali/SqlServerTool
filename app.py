...
class MainWindow(QMainWindow):
    ...
    def setup_db_manager_tab(self):
        ...
        self.list_databases.itemSelectionChanged.connect(self.on_db_selected)
    ...
    def _on_db_list_refreshed(self, dbs: list[DatabaseInfo]):
        ...
        for db in dbs:
            self.list_databases.addItem(db.name)
    ...
    def on_db_selected(self):
        if not self.list_databases.selectedItems(): return
        db_name = self.list_databases.selectedItems()[0].text()
    ...
    def _get_selected_db_name(self):
        items = self.list_databases.selectedItems()
        if not items:
